...
import { Grid, Sliders, Target, Layout } from 'react-feather';
// @ts-ignore
...
const ClickableCard: React.FC<ClickableCardProps> = ({ icon: Icon, title, value, onClick, color = "primary" }) => (
...
interface DocConfigHeaderProps {
    config: {
        framework: string;
        tone: string;
...
const DocConfigHeader: React.FC<DocConfigHeaderProps> = ({ config, onConfigChange, onTemplatesClick }) => {
    ...
    const [ambiguityConstraints, setAmbiguityConstraints] = useState<any[]>([]);
    ...
    const handleAmbiguitySelect = (constraints: any[]) => {
