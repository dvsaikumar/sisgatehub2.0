...
const CreateDoc: React.FC = () => {
    ...
    const [frameworkData, setFrameworkData] = useState<Record<string, any>>({});
    ...
    const [pendingSaveContent, setPendingSaveContent] = useState<PendingSaveContent>({ name: '', content: '' });
    ...
    const handleTemplateSelect = (template: any) => {
        if (template) {
            ...
            // Populate the goal with a starter prompt based on the template
            setGoal(`Draft a ${template.title} document for ${template.bestFor.toLowerCase()}. Key details: `);

            toast.success(`Template "${template.title}" applied! Goal updated.`);
        }
    };

    const handleFrameworkDataChange = (key: string, value: any) => {
...
Example format:
{
  ${fields.map(f => `"${f}": "Detailed content here..."`).join(',\n  ')}
    ...
    const handleDocumentTypeSelect = async (selectedType: any) => {
    ...
    const handleSaveCloud = (docName: string, content: string) => {
    ...
    const handleDownload = (docName: string, content: string) => {
        ...
        <div className={`hk-pg-body bg-white py-2 ${isMobile ? '' : 'overflow-hidden'}`} style={{ height: isMobile ? 'auto' : 'calc(100vh - 85px)' }}>
            <Container fluid className="px-xxl-5 h-100 d-flex flex-column">
                ...
                <div className="flex-grow-1" style={{ minHeight: 0, marginBottom: isMobile ? '180px' : '115px' }}>
                    ...
                    <Row className={`g-4 ${isMobile ? '' : 'h-100'}`}> {/* Increased gutter from g-3 to g-4 */}
                ...
                <div
                    ...
                    style={{
                        ...
                        left: 0,
                        right: 0,
                        bottom: isMobile ? '90px' : '60px', // Clears Mobile Nav
                        height: isMobile ? 'auto' : '70px',
                        paddingTop: '0rem',
                        paddingBottom: '0rem'
                    }}
                >
                    <div className="w-100 px-3 d-flex flex-row justify-content-between align-items-center gap-2">
                        <div className="text-muted fs-7 text-start">
