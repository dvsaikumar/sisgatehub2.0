...
import { Badge, Card, Col, Row, Button, Modal, Spinner, Offcanvas, Dropdown, Form, InputGroup, Table } from 'react-bootstrap';
import {
...
const LibraryList: React.FC<LibraryListProps> = ({ filter, searchTerm, toggleInfo, viewMode = 'grid' }) => {
    ...
    const [shareType, setShareType] = useState<string>('doc'); // 'doc' or 'pdf'
    ...
    const [templateBlob, setTemplateBlob] = useState<any>(null);
    const [templatePreviewUrl, setTemplatePreviewUrl] = useState<any>(null);
    const [templateHtml, setTemplateHtml] = useState<string>('');
    const [templateKeys, setTemplateKeys] = useState<any[]>([]);
    const [templateDelimiters, setTemplateDelimiters] = useState<any>({ start: '{', end: '}' });
    const [templateData, setTemplateData] = useState<any>({});
    const [selectedFillUser, setSelectedFillUser] = useState<any>(null);
    const previewContainerRef = React.useRef<any>(null);
                ...
                <div className="file-list-view p-3">
                    ...
                    {loading ? (
                        <div className="text-center py-5 w-100">
                            ...
                            <p className="mt-2 text-muted">Loading library...</p>
                        </div>
                    ) : sortedData.length === 0 ? (
                        <div className="text-center py-5 w-100">
                        ...
                        <div className="table-responsive">
                            <Table hover className="align-middle mb-0">
                                <thead className="bg-light">
                                    <tr>
                                        <th className="py-3 ps-4" style={{ width: '40%' }}>Name</th>
                                    ...
                                    {sortedData.map(item => {
                                        ...
                                        return (
                                            <tr key={item.id} onClick={() => handlePreview(item)} style={{ cursor: 'pointer' }} className="group">
                                                ...
                                                <td>
                                                    <small className="fw-bold text-muted">{getCategoryName(item.category_id)}</small>
                                                </td>
                                                <td>
                                                    <Badge bg="soft-secondary" text="dark" className="fw-bold px-2 py-1">
                                                        {item.type || 'General'}
                                                    </Badge>
                                                </td>
                                                <td className="text-end pe-4">
                                                    ...
                                                    >
                                                        <Star size={18} weight={item.is_starred ? 'fill' : 'regular'} color={item.is_starred ? '#f59e0b' : 'currentColor'} />
                                                    </button>
                                                </td>
                                            </tr>
                                        );
